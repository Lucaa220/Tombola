...
from messages import get_testo_tematizzato, get_feature_name
...
from variabili import is_admin, get_chat_id_or_thread, find_group, on_bot_added, premi_default, get_default_feature_states
from log import send_all_logs, send_logs_by_group, log_interaction, logstats, logactivity, logclean
...
async def settings_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_admin(update, context):
        if update.callback_query:
            await update.callback_query.answer(get_testo_tematizzato('impostazioni_solo_admin', tema), show_alert=True)
        else:
            await update.message.reply_text(get_testo_tematizzato('impostazioni_solo_admin', tema))
        return
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("🧙 Tema", callback_data='menu_tema'),
    ...
    text = get_testo_tematizzato('pannello_controllo', tema)
...
async def show_extraction_menu(query, chat_id_str, settings, tema): 
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = get_testo_tematizzato('descrizione_estrazione', tema)
    try:
        await query.edit_message_text(text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN_V2)
    except Exception as e:
        logger.error(f"Errore in show_extraction_menu: {e}")
        await query.answer(get_testo_tematizzato('errore_aggiornamento_menu', tema), show_alert=True)

async def show_admin_menu(query, chat_id_str, settings, tema): 
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = get_testo_tematizzato('descrizione_admin', tema)
    try:
        await query.edit_message_text(text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN_V2)
    except Exception as e:
        logger.error(f"Errore in show_admin_menu: {e}")
        await query.answer(get_testo_tematizzato('errore_aggiornamento_menu', tema), show_alert=True)

async def show_premi_menu(query, chat_id_str, settings, tema): 
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = get_testo_tematizzato('descrizione_premi', tema)
    try:
        await query.edit_message_text(text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN_V2)
    except Exception as e:
        if "Message is not modified" not in str(e):
            logger.error(f"Errore in show_premi_menu: {e}")
            await query.answer(get_testo_tematizzato('errore_aggiornamento_menu', tema), show_alert=True)

async def show_bonus_menu(query, chat_id_str: str, settings: dict, tema):
    group_conf = settings.setdefault(chat_id_str, {})  
    theme_defaults = get_default_feature_states(tema)
    group_overrides = group_conf.get("bonus_malus_settings", {})
    ...
    for key in ordered_feature_keys:
        if key in _ALL_BONUS_FEATURES:
            label = get_feature_name(key, tema)
    ...
    keyboard.append([InlineKeyboardButton("🔙 Indietro", callback_data='back_to_main_menu')])
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = get_testo_tematizzato('descrizione_bonus_malus', tema)
    try:
        await query.edit_message_text(text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN_V2)
    except Exception as e:
        if "Message is not modified" not in str(e):
...
async def show_delete_menu(query, chat_id_str: str, settings: dict, tema):
    delete_flag = settings.get(str(chat_id_str), {}).get('delete_numbers_on_end', False)
    keyboard = [
        [
            InlineKeyboardButton(f"Si {'✅' if delete_flag else ''}", callback_data='set_delete_yes'),
            InlineKeyboardButton(f"No {'✅' if not delete_flag else ''}", callback_data='set_delete_no')
        ],
        [InlineKeyboardButton("🔙 Indietro", callback_data='back_to_main_menu')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = get_testo_tematizzato('descrizione_elimina_numeri', tema)
    try:
        await query.edit_message_text(text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN_V2)
    except Exception as e:
        if "Message is not modified" not in str(e):
            logger.error(f"Errore in show_delete_menu: {e}")
            await query.answer(get_testo_tematizzato('errore_aggiornamento_menu', tema), show_alert=True)

async def show_tema_menu(query, chat_id_str, settings, tema):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    text = get_testo_tematizzato('descrizione_tema', tema)
    try:
        await query.edit_message_text(text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN_V2)
    except Exception as e:
        logger.error(f"Errore in show_tema_menu: {e}")
        await query.answer(get_testo_tematizzato('errore_aggiornamento_menu', tema), show_alert=True)
...
async def settings_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tema = settings.get(chat_id_str, {}).get('tema', 'normale')
    ...
    if action == 'menu_admin':
        await show_admin_menu(query, chat_id_str, settings, tema)
        logger.info(f"Azione {action} completata")
        return
    if action == 'menu_premi':
        await show_premi_menu(query, chat_id_str, settings, tema)
        logger.info(f"Azione {action} completata")
        return
    if action == 'menu_bonus':
        await show_bonus_menu(query, chat_id_str, settings, tema)
        logger.info(f"Azione {action} completata")
        return
    if action == 'menu_delete':
        await show_delete_menu(query, chat_id_str, settings, tema)
        logger.info(f"Azione {action} completata")
        return
    if action == 'menu_tema':
    ...
    if action.startswith("set_premio_"):
        ...
        if len(parts) != 4:
            logger.error(f"Formato action invalido: {action}")
            return
        premio_key = parts[2]
        change_str = parts[3]
        try:
            ...
            logger.error(f"Valore change invalido: {change_str}")
            return
    ...
    if action == 'close_settings':
        try:
            ...
            await query.answer(get_testo_tematizzato('impossibile_chiudere_menu', tema), show_alert=True)
        return
...
async def numero_giocatori(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not game.game_active:
        await update.message.reply_text(get_testo_tematizzato('nessuna_partita_attiva_per_giocatori', tema))
        return
    numero_giocatori_attivi = len(game.players)
    if numero_giocatori_attivi == 0:
        await update.message.reply_text(
            get_testo_tematizzato('nessun_giocatore_unito', tema),
            parse_mode=ParseMode.MARKDOWN_V2
        )
    else:
        await update.message.reply_text(
            get_testo_tematizzato('numero_giocatori_attivi', tema, count=numero_giocatori_attivi),
            parse_mode=ParseMode.MARKDOWN_V2
        )
...
async def classifica(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_admin(update, context):
        await update.message.reply_text(get_testo_tematizzato('classifica_solo_admin', tema))
        return
    ...
    if not classifica_gruppo:
        await context.bot.send_message(
            chat_id=chat_id,
            text=get_testo_tematizzato('nessuna_classifica_disponibile', tema),
    ...
    if lines:
        testo = get_testo_tematizzato('testo_classifica', tema, lines="\n".join(lines))
        await context.bot.send_message(chat_id=chat_id, text=testo, message_thread_id=thread_id, parse_mode=ParseMode.MARKDOWN_V2)
    else:
        await context.bot.send_message(chat_id=chat_id, text=get_testo_tematizzato('nessuna_classifica_disponibile', tema), message_thread_id=thread_id, parse_mode=ParseMode.MARKDOWN_V2)
...
async def combined_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rule_actions = action.startswith('rule_')

    if any(settings_actions):
        logger.debug(f"Routing action '{action}' a settings_button")
        await settings_button(update, context)
    elif rule_actions:
        logger.debug(f"Routing action '{action}' a rule_section_callback")
        await rule_section_callback(update, context)
    else:
        logger.debug(f"Routing action '{action}' a button (es. join_game)")
        await button(update, context)
    logger.info(f"Azione {action} completata in combined_button_handler")
...
async def main(polling: bool = False) -> None:
    ...
    application.add_handler(CommandHandler('logstats', logstats))
...
if __name__ == '__main__':
    ...
    asyncio.run(main(polling=args.polling))    "110": "110",
    "666": "666",
    "404": "404",
    "Tombolino": "Tombolino"
}
...
# --------------------------------------------------------------------
logging.basicConfig(
    level=logging.INFO,  # Cambiato a DEBUG per log dettagliati su updates raw
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s'
)
logger = logging.getLogger(__name__)
...
async def show_extraction_menu(query, chat_id_str, settings):
    ...
    try:
        ...
        logger.error(f"Errore in show_extraction_menu: {e}")
        await query.answer("Errore interno durante l'aggiornamento. Riprova.", show_alert=True)
