    ...
    if query.data == 'join_game':
        ...
        async with game.join_lock:
            ...
            try:
                ...
                logger.error(f"Errore invio cartella in privato a {user_id}: {e}")
                await context.bot.send_message(
                    chat_id=user_id,
            ...
            await query.answer()  # Ferma l'orologio senza notifica dopo successo
...
async def stop_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text="*⚠️ Il gioco è stato interrotto*",
        message_thread_id=thread_id,
        parse_mode=ParseMode.MARKDOWN_V2
    )
...
async def rule_section_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()  # Aggiunto per fermare l'orologio sui bottoni
